...
// List chứa ảnh đã xóa(thùng rác)
List deletedPhotos = NULL;

// Khai báo hàm xóa ảnh vĩnh viễn
List DeletePermanently(List l, int ID);

// Khai báo hàm xóa nhiều ảnh
...
/*Hàm tìm ảnh bằng thời gian và địa điểm*/
...
// Hàm khôi phục ảnh từ thùng rác
List Restore(List l, int ID) {
	...
	while (current) {
		if (current->ID == ID) {
			...
			current->next = NULL; // Gỡ ảnh khỏi danh sách đã xóa 
			l = InsertByID(l, current); // Add ảnh vào danh sách ảnh
...
int main() {
	...
	// Mảng gồm địa điểm và kích cỡ ảnh
	...
	// Tạo 100 ảnh có sẵn trong danh sách
	for (int i = 1; i <= 100; i++) {
		int hour = 12 + (i / 5) % 12;
		int minute = (i % 5) * 10;
		char time[20];
	...
	while (1) {
		...
		case 2: {
			...
			char size[20] = "1920x1080"; // Size mặc định
			char location[50];
